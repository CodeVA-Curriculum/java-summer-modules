...
This is fine, but it's not all that visually interesting. What if we used `circle()` instead to make hills? (Note: the example below only shows the changes to the `setup()` method, it does not show the complete program).
