...
...because the way it's written now, the `x` variable is only available inside the `setup()` method definition. This feature is called "scope"; variables can have *global* scope (meaning they are available across the whole program), or *local* scope (meaning they are available only in a specific section of the code).

For now, you should pretty much always declare your variables at the top of your program *before* the `setup()` method. That way, your variables will have *global* scope and you won't run into any frustrating errors:
